...
class Rooms:
    ...
    def add_message_to_room(self, room_name, username, message):
        ...
        if (username != "ADMIN" and username != "COMMAND"):
            ...
            self.rooms.get(room_name).add_message(username + ": " + message)
    ...
    def add_user(self, username):
        if ('General' not in self.rooms.keys()):
    ...
    def change_user_room(self, username, room):
        if (room in self.rooms):
            ...
            self.add_message_to_room(self.get_user_room(username), "ADMIN", "Room '" + room + "' does not exist.")

    # = Command Parsing =

    def parse_command(self, command, arguments, username):
        if (command == "help"):
            self.add_message_to_room(self.users[username], "COMMAND",
                                     "help, room (list/create/join) (room name), clear")
        elif (command == "room"):
            if (arguments[0] == "list"):
